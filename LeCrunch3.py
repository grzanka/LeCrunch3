...
setting_commands = (
    ["TIME_DIV", "COMM_FORMAT", "COMM_HEADER", "COMM_ORDER"]
...
class LeCrunch3(object):
    ...
    def __init__(self, host, port=1861, timeout=5.0):
        logging.info("Connecting to %s:%d with timeout %g s", host, port, timeout)
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.connect((host, port))
        self.sock.settimeout(timeout)
        logging.info("Connected to scope")
        self.clear()
    ...
    def clear(self, timeout=0.5):
        ...
        """
        logging.debug(
            "Clear any bytes in the oscilloscope's output queue by receiving packets until the connection blocks for more than `timeout` seconds."
        )
    ...
    def send(self, msg):
        ...
        """
        logging.debug("Format and send the string `%s`.", msg)
        if not msg.endswith("\n"):
            msg = str(msg) + "\n"
    ...
    def check_last_command(self):
        ...
        """
        logging.debug("Check that the last command sent was received okay")
        self.send("cmr?")
    ...
    def set_settings(self, settings):
        ...
        """
        logging.debug("Sends a `settings` dict of Command->Setting to the scope.")
    ...
    def get_channels(self):
        ...
        """
        channels = []
        logging.debug("Returns a list of the active channels on the scope.")
        for i in range(1, 5):
    ...
    def set_sequence_mode(self, nsequence):
        ...
        """
        logging.debug("Sets the scope to use sequence mode for aquisition.")
        if nsequence == 1:
            self.send("seq off")
        else:
    ...
    def get_wavedesc_frombuffer(self, msg):
        ...
        """
        logging.debug("Interpret wavedescription data")
        data = io.BytesIO(msg)
    ...
    def get_wavedesc(self, channel):
        ...
        dictionary format.
        """
        logging.debug("Requests the wave descriptor for `channel` from the scope")
    ...
    def get_waveform_all(self, channel):
        logging.debug("Get data from all waveforms")
        if channel not in range(1, 5):
            raise Exception("channel must be in %s." % str(range(1, 5)))
        ...
        waveform = np.frombuffer(
            ...
            wavedesc["dtype"],
            wave_array_count // wavedesc["dtype"].itemsize,
        )
    ...
    def get_waveform(self, channel):
        ...
        scope readout.
        """
        logging.debug("Capture the raw data for channel %d from the scope", channel)
        ...
        return (
            wavedesc,
            np.frombuffer(msg[22:], wavedesc["dtype"], wavedesc["wave_array_1"]),
        )
